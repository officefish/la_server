...
class LobbyHandler(websocket.WebSocketHandler):
    ...
    def on_message(self, message):
        ...
        if type == 'connect_to_lobby':
            ...
            LobbyHandler.init_player(
                self.id, self.level, self.deck_id, self.hero_id, self.hero_uid, self.nickname)
            ...
            for player in LobbyHandler.handlers:
                players.append({'id': player.id, 'uid': player.hero_uid,
                                'level': player.level, 'nickname': player.nickname})
            ...
            redis_client.hset(match, 'player1_deckId',
                              self.player1Data['deckId'])
            redis_client.hset(match, 'player2_deckId',
                              self.player2Data['deckId'])

            redis_client.hset(match, 'player1_level',
                              self.player1Data['level'])
            redis_client.hset(match, 'player2_level',
                              self.player2Data['level'])

            redis_client.hset(match, 'player1_heroId',
                              self.player1Data['heroId'])
            redis_client.hset(match, 'player2_heroId',
                              self.player2Data['heroId'])
    ...
    def init_player(cls, id, level, deckId, heroId, hero_uid, nickname):
        cls.players[id] = {'level': level, 'deckId': deckId,
                           'heroId': heroId, 'hero_uid': hero_uid, 'nickname': nickname}
