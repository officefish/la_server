...
class Controller ():
    ...
    def activateDynamic(self, unit):
        for eptitude in unit.eptitudes:
            ...
            if eptitude.dynamic:
                ...
                if eptitude.type == EptitudeType.INCREASE_ATTACK_MIXIN:
                    logger.debug('activateDynamic: for %s cards' %
                                 (len(targets)))
                    self.increase_attack_mixin(targets, eptitude)
    ...
    def deactivateTempEptitudes(self, unit):
        ...
        for eptitude in unit.tempEptitudes:
            ...
            if eptitude.lifecycle < 0:
                ...
                # пример: мана которая работает на один ход для обоих
                # персонажей
                pass
    ...
    def configure(self):
        ...
        if len(self.eptitudes):
            ...
            targets = self.match.getLevelTargets(
                self.unit, eptitude, self.whiteFlag)
            ...
            if period == eptitude.period:
                ...
                if isinstance(eptitude.dependency, int):
                    if eptitude.dependency > 0:
                        logger.debug(
                            'Init eptitude which has dependency from another eptitude: %s' % eptitude.dependency)
                        dependencyEptitude = self.unit.getEptitudeById(
                            eptitude.dependency)
                        logger.debug('dependency.activated: %s' %
                                     dependencyEptitude.activated)
                ...
                if eptitude.probability < 100:
                    logger.debug(
                        'Probability of eptitude activation %s per.' % eptitude.probability)
                ...
                if len(targets) and eptitude.activate_widget == True:
                    if not self.unit.hasSelfDieEptitude():
                        logger.debug('Activate Widget for %s' %
                                     self.unit.cardData['title'])
                        targetAttachment = self.match.initAttachment(
                            self.unit, self.whiteFlag)
                        targetIndex = self.match.initIndex(
                            self.unit, targetAttachment, self.whiteFlag)
                        action = {}
                ...
                if eptitude.type == EptitudeType.JERK:
                    ...
                    action['client'] = self.client
                    action['endAnimationFlag'] = False
                    clientTargets = self.match.getTargetsCoord(
                        jerkTargets, self.whiteFlag)
                ...
                if eptitude.type == EptitudeType.PASSIVE_ATTACK:
                    ...
                    targets = self.match.getLevelTargets(
                        self.unit, eptitude, self.unit.getWhiteFlag())
                ...
                if eptitude.type == EptitudeType.PASSIVE_ATTACK_FOR_SEVERAL_TARGETS:
                    logger.debug(
                        'eptitude.type: PASSIVE_ATTACK_FOR_SEVERAL_TARGETS')
                    self.passive_attack_for_several_targets(
                        self.unit, eptitude)
                ...
                if eptitude.type == EptitudeType.INCREASE_HEALTH:
                    ...
                    for target in targets:
                        ...
                        targetAttachment = self.match.initAttachment(
                            target, self.whiteFlag)
                        targetIndex = self.match.initIndex(
                            target, targetAttachment, self.whiteFlag)
                ...
                if eptitude.type == EptitudeType.PICK_CARDS_DEPENDS_ON_OPPONENT_CARDS_COUNT:
                    ...
                    self.pick_card_depends_on_opponent_cards_count(
                        targets, eptitude)
                ...
                if eptitude.type == EptitudeType.MASSIVE_ATTACK_DEPENDS_ON_TARGET_ATTACK_VALUE:
                    logger.debug(
                        'eptitude.type: MASSIVE_ATTACK_DEPENDS_ON_TARGET_ATTACK_VALUE')
                    self.massive_attack_depends_on_target_attack_value(
                        targets, eptitude)

                if eptitude.type == EptitudeType.MASSIVE_ATTACK_DEPENDS_ON_UNIT_ATTACK_VALUE:
                    logger.debug(
                        'eptitude.type: MASSIVE_ATTACK_DEPENDS_ON_UNIT_ATTACK_VALUE')
                    targets = self.match.getLevelTargets(
                        self.unit, eptitude, self.unit.getWhiteFlag())
                ...
                if eptitude.type == EptitudeType.INCREASE_ATTACK_AND_HEALTH_DEPENDS_ON_TOKENS:
                    logger.debug(
                        'eptitude.type: INCREASE_ATTACK_AND_HEALTH_DEPENDS_ON_TOKENS')
                ...
                if eptitude.type == EptitudeType.INCREASE_CARD_PRICE_MIXIN:
                    ...
                    self.increase_card_price_mixin(
                        self.unit, targets, eptitude)

                if eptitude.type == EptitudeType.DECREASE_CARD_PRICE_MIXIN:
                    logger.debug('eptitude.type: DECREASE_CARD_PRICE_MIXIN')
                    self.decrease_card_price_mixin(
                        self.unit, targets, eptitude)

                if eptitude.type == EptitudeType.INCREASE_HEALTH_DEPENDS_ON_ASSOCIATE_CARDS:
                    logger.debug(
                        'eptitude.type: INCREASE_HEALTH_DEPENDS_ON_ASSOCIATE_CARDS')
                    self.increase_health_depends_on_associate_cards(
                        targets, eptitude)

                if eptitude.type == EptitudeType.INCREASE_HEALTH_DEPENDS_ON_OPPONENT_CARDS:
                    logger.debug(
                        'eptitude.type: INCREASE_HEALTH_DEPENDS_ON_OPPONENT_CARDS')
                    self.increase_health_depends_on_opponent_cards(
                        targets, eptitude)
                ...
                if eptitude.type == EptitudeType.ATTACK_RANDOM_UNIT:
                    ...
                    if len(list) > 1:
                        # TODO and here we should add activate animation action
                        # with self.unit to scenario
                        targetAttachment = self.match.initAttachment(
                            self.unit, self.whiteFlag)
                        targetIndex = self.match.initIndex(
                            self.unit, targetAttachment, self.whiteFlag)
                ...
                if eptitude.type == EptitudeType.PRIMARY_TARGET:
                    ...
                    if raceFlag:
                        try:
                            logger.debug('attackUnit.race.id:%s , raceId:%s' % (
                                self.match.attackUnit.race.id, raceId))
    ...
    def decreaseHealth(self, targets, eptitude):
        ...
        for target in targets:
            ...
            if target.maxHealth == 0:
                ...
                action['client'] = self.client
                action['targets'] = [
                    {'index': targetIndex, 'attachment': targetAttachment, 'damage': power}]
                self.scenario.append(action)

                self.match.deleteUnit(
                    targetIndex, targetAttachment, self.whiteFlag)
                action = {}
    ...
    def shuffleUnitToDeck(self, targets, eptitude):
        ...
        for target in targets:
            ...
            units.append(
                {'index': targetIndex, 'attachment': targetAttachment, 'card': cardData})
    ...
    def dropCard(self, targets, eptitude):
        ...
        for i in range(count):
            if len(indexes) < len(targetHand):
                ...
                attachment, index = self.match.initCardAttachment(
                    card, self.whiteFlag)
                cardData = {}
                cardData['card'] = card
    ...
    def generateCard(self, targets, eptitude, type):
        ...
        for unit in row:
            Controller(self).newCard(unit)
            if cardData['whiteFlag'] == unit.whiteFlag:
                Controller(self).newPlayerCard(unit)
            else:
                Controller(self).newOpponentCard(unit)
    ...
    def attach_eptitude(self, targets, eptitude):
        ...
        for target in targets:
            ...
            Controller(self).activateEptitude(target)
    ...
    def copy_unit(self, targets, eptitude):
        for target in targets:
            ...
            Controller(self).addUnit(targetUnit)

            Controller(self).unitPlaced(targetUnit)
    ...
    def unit_convertion(self, targets, eptitude):
        for target in targets:
            ...
            Controller(self).setWhiteFlag(target.whiteFlag).configureUnit(convertionUnit)
    ...
    def entice_unit(self, targets, eptitude):
        for target in targets:
            ...
            if len(opponentRow) < 7:
                ...
                for unit in row:
                    Controller(self).enticeAssociate(unit)

                for unit in opponentRow:
                    Controller(self).enticeOpponent(unit)
    ...
    def replace_card_and_token(self, targets, eptitude):
        ...
        if len(targets):
            ...
            initiatorAttachment = self.match.initAttachment(
                self.unit, self.whiteFlag)
            initiatorIndex = self.match.initIndex(
                self.unit, initiatorAttachment, self.whiteFlag)
            initiatorCard = self.match.copyCard(self.unit.cardData)
            self.match.deleteUnit(
                initiatorIndex, initiatorAttachment, self.whiteFlag)

            card = targets[0]
            cardAttachment, cardIndex = self.match.initCardAttachment(
                card, self.whiteFlag)
        ...
        for i in range(eptitude.count):
            ...
            if eptitude.attachment == EptitudeAttachment.ASSOCIATE:
                ...
                index = random.randint(
                    0, len(graveyard) + len(opponentGraveyard) - 1)
    ...
    def beckCardToHand(self, targets, eptitude):
        ...
        for target in targets:
            ...
            selectAttachment = self.match.initAttachment(
                target, self.whiteFlag)
            selectIndex = self.match.initIndex(
                target, selectAttachment, self.whiteFlag)
    ...
    def backSeveralTokensToHand(self, targets, eptitude):
            ...
            if target.whiteFlag == self.whiteFlag:
                if eptitude.attachment == EptitudeAttachment.ASSOCIATE or eptitude.attachment == EptitudeAttachment.ALL:
                    ...
                    if handLength < 10:
                        ...
                        associateBackCardsData.append(
                            {'index': targetIndex, 'attachment': targetAttachment, 'card': cardData, 'cardIndex': cardIndex})
                    else:
                        destroyCards.append(target)
                        destroyCardsData.append(
                            {'index': targetIndex, 'attachment': targetAttachment, 'damage': target.getHealth()})
            else:
        ...
        for targetUnit in destroyCards:
            Controller(self).removeUnit(targetUnit)
    ...
    def increase_a_h_depepend_on_tokens(self, targets, eptitude):
        ...
        for targetUnit in targets:
            ...
            targetIndex = self.match.initIndex(
                targetUnit, targetAttachment, self.whiteFlag)
            action['targetIndex'] = targetIndex
            action['targetAttachment'] = targetAttachment
            self.scenario.append(action)
    ...
    def default_attack(self, targets, eptitude):
        for targetUnit in targets:
            ...
            index = self.match.initIndex(
                targetUnit, attachment, self.whiteFlag)
            action = {}
            action['type'] = Action.INCREASE_ATTACK
            action['client'] = self.client
            action['endAnimationFlag'] = True
            ...
            if targetUnit.getHealth() == targetUnit.maxHealth:
                ...
                if isinstance(targetUnit, HeroUnit):
                    if targetUnit == playerHero:
                        Controller(self).treatHero(playerRow, True)
                        Controller(self).treatHero(opponentRow, False)
                    else:
                        Controller(self).treatHero(playerRow, False)
                        Controller(self).treatHero(opponentRow, True)

                    Controller(self).treatHeroes(playerRow, opponentRow)
                else:
                    Controller(self).treatAssociate(playerRow)
                    Controller(self).treatOpponent(opponentRow)
                    Controller(self).treatAll(playerRow, opponentRow)

                if isinstance(targetUnit, Unit):
                    if targetUnit.getHealth() == targetUnit.getMaxHealth():
                        Controller(self).fullHealthUnit(targetUnit)
    ...
    def kill(self, unit, targets, eptitude):
        ...
        for targetUnit in targets:
            ...
            if targetIndex >= 0:
                if isinstance(targetUnit, Unit):
                    Controller(self).woundUnit(targetUnit)

                action = {}
                action['type'] = Action.DAMAGE
                action['client'] = self.client
                action['targets'] = [
                    {'index': targetIndex, 'attachment': targetAttachment, 'damage': targetUnit.getHealth()}]
                self.scenario.append(action)
    ...
    def passive_attack_series(self, unit, eptitude):
        ...
        attackValue = 1
        for i in range(count):
            targetUnit = self.match.getLevelTargets(
                unit, eptitude, unit.whiteFlag)[0]
            ...
            initiatorAttachment = self.match.initAttachment(
                unit, self.whiteFlag)
            initiatorIndex = self.match.initIndex(
                unit, initiatorAttachment, self.whiteFlag)
            ...
            if not shieldActionFlag:
                ...
                action['client'] = self.client
                action['targets'] = [
                    {'index': targetIndex, 'attachment': targetAttachment, 'damage': attackValue}]
                self.scenario.append(action)
    ...
    def getNotSameRandow(self, unit, eptitude, targets):
        levelTargets = self.match.getLevelTargets(
            unit, eptitude, unit.whiteFlag)
    ...
    def passive_attack_for_several_targets(self, unit, eptitude):
        ...
        for i in range(eptitude.count):
            ...
            targetData['index'] = self.match.initIndex(
                target, attachment, self.whiteFlag)
    ...
    def massive_attack(self, unit, targets, eptitude):
        ...
        for targetUnit in targets:
            ...
            if targetUnit.shield:
                ...
                scenarioTargets.append(
                    {'index': index, 'attachment': attachment, 'damage': 0, 'health': targetUnit.getHealth()})
            else:
                targetHealthValue = targetUnit.getHealth()
                targetNewHealthValue = targetHealthValue - attackValue
                targetUnit.setHealth(targetNewHealthValue)
                scenarioTargets.append({'index': index, 'attachment': attachment,
                                        'damage': attackValue, 'health': targetUnit.getHealth()})
                ...
                if self.match.deckLength(unitFlag):
                    if len(hand) < 10:
                        ...
                        if self.match.getMode() == 2:
                            ...
                            if self.whiteFlag == unitFlag:
                                action[
                                    'attachment'] = EptitudeAttachment.ASSOCIATE
                            else:
                                action['attachment'] = EptitudeAttachment.OPPONENT
                            self.scenario.append(action)
                    else:
            ...
            if len(hand) < eptitude.power:
                ...
                for i in range(count):
                    ...
                    if self.match.deckLength(target.whiteFlag):
                        if len(hand) < 10:
                            ...
                            if self.match.getMode() == 2:
                                ...
                                if self.whiteFlag == target.whiteFlag:
                                    ...
                                    action[
                                        'attachment'] = EptitudeAttachment.OPPONENT
                                self.scenario.append(action)
                        else:
                            if self.match.burnExtraCardsFlag:
                                card = self.match.getCard(
                                    target.whiteFlag, False)
                                cardCopy = self.match.copyCard(card)
                                action = {}
                                action['type'] = Action.BURN_CARD
                                if self.whiteFlag == target.whiteFlag:
                        ...
                        if self.match.attritionFlag:
                            self.match.attrition(
                                self.client, target.whiteFlag, False)
    ...
    def new_unit(self, unit, targets, eptitude):
        ...
        for i in range(count):
            ...
            if eptitude.price > - 1:
                try:
                    unitCard = random.choice(Card.objects.filter(
                        price=eptitude.price, type=CardType.UNIT))
    ...
    def increase_card_price(self, unit, targets, eptitude):
        ...
        logger.debug('controller.increase_card_price for cards %s' %
                     len(targets))
        ...
        for card in targets:
            ...
            if index > - 1:
                ...
                logger.debug('card.attachment :%s, card.index:%s' %
                             (attachment, index))
                card['price'] += value
    ...
    def decrease_card_price(self, unit, targets, eptitude):
        value = eptitude.power
        logger.debug('controller.decrease_card_price for cards %s' %
                     len(targets))
