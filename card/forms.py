...
class CardForm (forms.Form):
    ...
    type = forms.ChoiceField(choices=CARD_TYPE_CHOISES,
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['race'] = forms.ChoiceField(
            required=False, choices=race_choises)

        subrace_choises = [(-1, '----')]
        subrace_choises += [(o.id, str(o.title))
                            for o in SubRace.objects.all()]

        self.fields['subrace'] = forms.ChoiceField(
            required=False, choices=subrace_choises)

        book_choises = [(-1, '----')]
        book_choises += [(o.id, o.title) for o in Book.objects.all()]
        self.fields['book'] = forms.ChoiceField(
            required=False, choices=book_choises)

        group_choises = [(-1, '----')]
        group_choises += [(o.id, o.title) for o in Group.objects.all()]
        self.fields['group'] = forms.ChoiceField(
            required=False, choices=group_choises)
...
class EptitudeForm (forms.Form):
    ...
    power = forms.IntegerField(
        max_value=30, initial=0, required=False, widget=forms.TextInput(attrs={'maxlength': 2}))
    max_power = forms.IntegerField(
        max_value=30, initial=0, required=False, widget=forms.TextInput(attrs={'maxlength': 2}))
    count = forms.IntegerField(
        max_value=30, initial=0, required=False, widget=forms.TextInput(attrs={'maxlength': 2}))
    lifecycle = forms.IntegerField(
        required=False, initial=0, widget=forms.TextInput(attrs={'maxlength': 1}))
    probability = forms.IntegerField(
        min_value=0, max_value=100, initial=100, widget=forms.TextInput(attrs={'maxlength': 3}))
    ...
    manacost = forms.IntegerField(
        min_value=0, max_value=10, initial=0, widget=forms.TextInput(attrs={'maxlength': 2}))
    ...
    def __init__(self, card, *args, **kwargs):
        ...
        self.fields['unit'] = forms.ChoiceField(
            required=False, choices=unit_choises)
        ...
        self.fields['price'] = forms.IntegerField(
            required=False, initial=-1, widget=forms.TextInput(attrs={'maxlength': 2}))

        eptitude_choises = [(-1, '----')]
        eptitude_choises += [(o.id, etc[o.type]) for o in card.eptitudes]
        self.fields['dependency'] = forms.ChoiceField(
            required=False, choices=eptitude_choises)
        attach_eptitude_choises = [(-1, '----')]
        attach_eptitude_choises += [(o.id, etc[o.type])
                                    for o in card.eptitudes]
        self.fields['attach_eptitude'] = forms.ChoiceField(
            required=False, choices=attach_eptitude_choises)

        weapon_choises = [(-1, '----')]
        weapon_choises += [(o.id, str(o.title)) for o in Weapon.objects.all()]
        self.fields['weapon'] = forms.ChoiceField(
            required=False, choices=weapon_choises)
